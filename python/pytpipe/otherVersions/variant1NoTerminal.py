...
def runPipeline(pipeline):
    print("Run pipeline with pipeline", pipeline)
    # Splitting string into list:
    commandList = pipeline.split()
    print("Commandlist: ", commandList)

    if len(commandList) < 2:
        print("You have too few arguments. You must have at least one valid command", file=sys.stderr)
        sys.exit(1)
    ...
    while i < len(commandList):
        ...
        # Take all arguments, until "|" or list is empty

        arguments = []
        for arg in commandList[startArg:]:
            print("Start argument: ", startArg)
            print("Arg = ", arg)
            if arg == ">":
                endPrint = True
                break            
            elif arg == "sep" or i == len(commandList) - 1:
                break
            arguments.append(arg)
        
        currentString = functionCall(command, currentString, *arguments)
        print("Currentstring in while: ", currentString)
        numberOfArguments = len(arguments)
        print("numberOfArguments: ", numberOfArguments)
        #Surpassing the command, the number of arguments and the separator
        i += numberOfArguments + 2
    if(currentString == ""):
        # Shoudl be error!
        return "The pipeline had no output!"
    else:
        return currentString

def main():
    pipeline = "cat ./test/listeAf5Hej.txt sep grep hej"
    result = runPipeline(pipeline)
    print(result)
